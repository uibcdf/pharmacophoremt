...
# ------------------------------------------------------------------------------
# PharmacophoreMT development bootstrap with activation switch & flexible path discovery
# - Creates/updates an environment from development_env.yaml
# - Installs PharmacophoreMT in editable mode (no deps)
...
#     * parent-of-repo, repo root, pharmacophoremt/, or devtools/
#
# Usage examples:
...
Options:
  ...
  --env-name <name>
      Environment name (default: pharmacophoremt)

  --env-yaml <path>
      Path to the YAML file. If omitted, the script auto-discovers it
      (works when called from parent-of-repo, repo root, pharmacophoremt/, or devtools/).
...
CWD_CANDIDATES=(
  "pharmacophoremt/devtools/conda-envs/development_env.yaml"
...
SCRIPT_CANDIDATES=(
  "${REPO_ROOT}/pharmacophoremt/devtools/conda-envs/development_env.yaml"
...
# --- Editable install -----------------------------------------------------------
echo "[setup] Installing PharmacophoreMT in editable mode (no deps)..."
${RUN} python -m pip install --no-deps --editable .

# --- Optional kernel registration ----------------------------------------------
if [[ "${REGISTER_KERNEL}" == "1" ]]; then
  echo "[setup] Registering Jupyter kernel 'Python (pharmacophoremt)'..."
  ${RUN} python -m ipykernel install --user --name "${ENV_NAME}" --display-name "Python (pharmacophoremt)"
fi
