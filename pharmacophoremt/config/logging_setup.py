...
def setup_logging(
    ...
    *,
    ...
    logger_name: str = "pharmacophoremt",
) -> logging.Logger:
    """
    Configure PharmacophoreMT logging and (optionally) capture Python warnings.

    Effects
    -------
    - Creates/gets logger `logger_name` (default: "pharmacophoremt")
    - Attaches a StreamHandler if not present, with formatter:
          "PHARMACOPHOREMT %(levelname)s | %(message)s"
    ...
    if stream_handler is None:
        ...
        # "PHARMACOPHOREMT WARNING | <Category>: <message>"
        # (For normal logs, <Category> won't appear, but the prefix still matches.)
        formatter = logging.Formatter("PHARMACOPHOREMT %(levelname)s | %(message)s")
