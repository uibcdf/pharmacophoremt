...
def from_ligandscout(pharmacophore):
    ...
    for feature_xml in root.findall(".//feature"):
        ...
        center = puw.quantity(
            [float(pos_xml.get("x")), float(pos_xml.get("y")), float(pos_xml.get("z"))],
            "angstroms",
        )
        ...
        if dir_xml is not None and phmt_feature in [
            "hb donor",
            "hb acceptor",
            "aromatic ring",
        ]:
            ...
            if phmt_feature == "hydrophobicity":
                ...
                site = interaction_sites.InteractionSite(
                    Sphere(center, radius), phmt_feature
                )
...
def to_ligandscout(pharmacophore, file_name=None):
    ...
    root.set(
        "name", pharmacophore.name if pharmacophore.name else "PharmacophoreMT Export"
    )

    for i, site in enumerate(pharmacophore.interaction_sites):
        ls_type = PHMT_TO_LS.get(
            site.feature_name, "H"
        )  # Default to Hydrophobic if unknown
