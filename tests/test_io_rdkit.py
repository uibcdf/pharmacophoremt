...
def test_rdkit_conversion():
    ...
    from pharmacophoremt.interaction_site import AromaticRingSphere, HBDonorSphere

    ph.add_interaction_site(
        HBDonorSphere(
            center="[1, 2, 3] angstroms", radius="1.0 angstroms", skip_digestion=True
        )
    )
    ph.add_interaction_site(
        AromaticRingSphere(
            center="[5, 5, 5] angstroms", radius="1.5 angstroms", skip_digestion=True
        )
    )
