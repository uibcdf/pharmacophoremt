...
def test_pharmacophore_basic_management():
    ...
    site = PositiveChargeSphere(
        center="[1, 1, 1] nm", radius="0.5 nm", skip_digestion=True
    )
...
def test_pharmacophore_get_quantities():
    ...
    ph.add_interaction_site(
        PositiveChargeSphere(
            center="[0, 0, 0] nm", radius="1.0 nm", skip_digestion=True
        )
    )
    ph.add_interaction_site(
        PositiveChargeSphere(
            center="[1, 1, 1] nm", radius="1.0 nm", skip_digestion=True
        )
    )
...
def test_pharmacophore_distance_matrix():
    ...
    ph.add_interaction_site(
        PositiveChargeSphere(
            center="[1, 0, 0] nm", radius="1.0 nm", skip_digestion=True
        )
    )
